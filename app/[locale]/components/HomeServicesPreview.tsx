  ...
  return (
    ...
    >
      <div className="container-custom relative">
        ...
        {/*
          Mobil: görsel üstte + altında 2×2 kart.
          lg+: tek grid — görsel col1 row-span-2 ⇒ yükseklik tam olarak iki satır kart (2 üst + 2 alt) ile aynı.
          Görsel: border yok, kart çerçevesi gibi değil; hafif gölge + yumuşak köşe, foto alanda “serbest”.
        */}
        <div className="mt-8 flex w-full max-w-full min-w-0 flex-col gap-8 overflow-x-clip lg:mt-10 lg:grid lg:grid-cols-[minmax(240px,0.36fr)_1fr_1fr] lg:grid-rows-2 lg:gap-x-7 lg:gap-y-4 xl:gap-x-9">
          ...
          <div className="grid min-w-0 grid-cols-2 gap-3 sm:gap-4 lg:contents lg:gap-0">
            {items.map((s) => (
              <Link
                key={s.id}
                href={`/services#${s.id}`}
                className="service-card-glow service-card-glow--home-preview flex min-h-0 min-w-0 flex-col rounded-[14px] border border-white/[0.07] bg-[#111827]/95 p-4 no-underline backdrop-blur-[2px] transition-colors hover:border-white/[0.12] hover:bg-[#141B2D] sm:p-5 lg:min-h-0 lg:p-4 xl:p-5"
              >
